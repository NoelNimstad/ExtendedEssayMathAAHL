...
#define MAX_VELOCITY        4       // honestly, just guess here, though there may be a smart way to calculate it
...
void hsv2rgb(float H, float *red, float *green, float *blue) // https://github.com/Inseckto/HSV-to-RGB/blob/master/HSV2RGB.c
{
...
void tick(state_t *state)
{
    ...
    for(size_t i = 0; PARTICLE_COUNT > i; i++)
    {
        ...
        SDL_RenderPoint(state->renderer, state->particles[i].position_x - 1, state->particles[i].position_y);   // star
        SDL_RenderPoint(state->renderer, state->particles[i].position_x, state->particles[i].position_y - 1);
        SDL_RenderPoint(state->renderer, state->particles[i].position_x, state->particles[i].position_y);
        SDL_RenderPoint(state->renderer, state->particles[i].position_x, state->particles[i].position_y + 1);
        SDL_RenderPoint(state->renderer, state->particles[i].position_x + 1, state->particles[i].position_y);
    }
