...
\begin{defn}\label{definition:VP} % velocity potential
	...
	property of the idealisations made in this essay\referto{Section}{section:KELVIN}) and is essential to its analysis.
\end{defn}

\subsubsection{Notation}
Vector calculus, like one-variable calculus, has no standardised notation. This essay will employ the following
notation:
\begin{itemize}
	\item $\nabla$:
	\begin{itemize}
...
\begin{theorem}[The extreme value theorem]\label{lemma:MINMAX}
...
\end{theorem}
\begin{theorem}[Rolle's theorem]\label{lemma:ROLLES}
	...
	\end{proof}
\end{theorem}
\begin{theorem}[The mean value theorem]\label{lemma:MVT}
	...
	\begin{proof}
		...
		$$
		Since $g(a)=g(b)=0$, Theorem~\ref{lemma:ROLLES} asserts that there is at least one point $c\in(a,b)$ such that $g'(c)=0.$ Thus, at $c$,
		\begin{align*}
