...
\end{defn}
\begin{lemma}[Clairaut's theorem]\label{lemma:CLAIRAUT}
...
\end{lemma}
\begin{defn}
	The \definedterm{nabla} or \definedterm{gradient} operator $\nabla$ is a vector containing one partial derivative for each
	parameter of the scalar valued function applied to \cite{RAPP2017137}. Thus applying the operator is taking the product of
	the vector $\nabla$ and a scalar valued function $f$. For some function $f:\mathbb{R}^n\rightarrow\mathbb{R}$, $\nabla$ and $\nabla f$
	would be given by:
	$$
	\nabla=\begin{bmatrix}
		\pdv*{x_1}\\
